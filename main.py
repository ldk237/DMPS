...
class StorageVirtualNode:
    def __init__(
        ...
        self.network_utilization = 0.0  # Current bandwidth usage in bps
    ...
    def add_connection(self, node_id: str, bandwidth: int):
        """Add a network connection to another node (bandwidth in Mbps)"""
    ...
    def process_chunk_transfer(
        ...
        # Convert chunk size to bits
        chunk_size_bits = chunk.size * 8
        
        # available bandwidth between source and this node (in bps)
        available_bandwidth = min(
            max(self.bandwidth - self.network_utilization, 0),
        ...
        # Make the sleep small if transfer_time is too tiny; also cap to reasonable max for demo
        simulated_sleep = min(max(transfer_time, 0.001), 0.5)
        ...
        # Increase network utilization temporarily (simulate short spike)
        self.network_utilization += available_bandwidth * 0.5
        # decrement it a bit to simulate release
        self.network_utilization = max(self.network_utilization - available_bandwidth * 0.4, 0)
